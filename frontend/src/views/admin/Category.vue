<template>
  <div class="page-wrap card">
    ...
    <el-dialog :title="ed.id?'编辑分类':'新增分类'" v-model="dlg" width="460px">
...
import { getCategories, saveCategory, updateCategory, deleteCategory } from '@/api/category'
const list=ref([]),ld=ref(false),sv=ref(false),dlg=ref(false),fr=ref(null)
const ed=reactive({id:null,name:'',description:'',sortOrder:0})
const r={name:[{required:true,message:'请输入名称'}]}
onMounted(load)
async function load(){ld.value=true;try{list.value=(await getCategories()).data}finally{ld.value=false}}
function open(row){Object.assign(ed,row?{...row}:{id:null,name:'',description:'',sortOrder:0});dlg.value=true}
async function save(){if(!(await fr.value.validate().catch(()=>false)))return;sv.value=true;try{ed.id?await updateCategory(ed):await saveCategory(ed);ElMessage.success('已保存');dlg.value=false;load()}finally{sv.value=false}}
async function del(id){try{await deleteCategory(id);ElMessage.success('已删除');load()}catch{ElMessage.error('失败')}}
</script>
