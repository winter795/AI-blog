...
import { submitComment, getComments } from '@/api/site'

const props = defineProps({ articleId: Number })
const list = ref([]), submitting = ref(false)
const f = reactive({ author: '', email: '', content: '' })
...
async function submit() {
  ...
  try {
    await submitComment({ articleId: props.articleId, author: f.author, email: f.email, content: f.content })
    ElMessage.success('提交成功，等待审核'); f.author = ''; f.email = ''; f.content = ''
...
.cs-form .el-button { display: block; margin: 0 auto; }
</style>
